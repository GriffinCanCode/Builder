...
struct FSharpConfig
{
    ...
    const(DotnetConfig) dotnet() const { return build.dotnet; }
    ref FAKEConfig fake() return { return build.fake; }
    ref FSIConfig fsi() return { return build.fsi; }
    const(FSIConfig) fsi() const { return build.fsi; }
    ref FableConfig fable() return { return build.fable; }
    const(FableConfig) fable() const { return build.fable; }
    ref NativeAOTConfig native() return { return build.native; }
    const(NativeAOTConfig) native() const { return build.native; }
    ...
    const(string[]) compilerFlags() const { return build.compilerFlags; }
    ...
    bool debug_() const { return build.debug_; }
    ref bool optimize() return { return build.optimize; }
    bool optimize() const { return build.optimize; }
