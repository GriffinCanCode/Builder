...
class TypeScriptHandler : BaseLanguageHandler
{
    ...
    private LanguageBuildResult buildExecutable(Target target, WorkspaceConfig config, TSConfig tsConfig)
    {
        LanguageBuildResult result;
        ...
        {
            installDependencies(target, tsConfig);
        }
    ...
    private LanguageBuildResult buildLibrary(Target target, WorkspaceConfig config, TSConfig tsConfig)
    {
    ...
    private LanguageBuildResult runTests(Target target, WorkspaceConfig config, TSConfig tsConfig)
    {
        LanguageBuildResult result;
    ...
    private LanguageBuildResult buildCustom(Target target, WorkspaceConfig config, TSConfig tsConfig)
    {
        LanguageBuildResult result;
        result.success = true;
        result.outputHash = FastHash.hashStrings(target.sources);
    ...
    private LanguageBuildResult compileTarget(Target target, WorkspaceConfig config, TSConfig tsConfig)
    {
        LanguageBuildResult result;
    ...
    private void installDependencies(Target target, TSConfig config)
    {
        string packageJsonPath = findPackageJson(target.sources);
