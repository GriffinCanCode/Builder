module runtime.hermetic.monitor;

import std.datetime : Duration, msecs;
import core.time : MonoTime;
import runtime.hermetic.spec : ResourceLimits;
import distributed.protocol.protocol : ResourceUsage;
...
ResourceMonitor createMonitor(ResourceLimits limits) @safe
{
    version(linux)
    {
        import runtime.hermetic.monitor.linux : LinuxMonitor;
        return new LinuxMonitor(limits);
    }
    else version(OSX)
    {
        import runtime.hermetic.monitor.macos : MacOSMonitor;
        return new MacOSMonitor(limits);
    }
    else version(Windows)
    {
        import runtime.hermetic.monitor.windows : WindowsMonitor;
